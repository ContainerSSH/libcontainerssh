...
func (m *metricsNetworkHandler) OnAuthPubKey(username string, pubKey string, clientVersion string, caCert *sshserver.CACertificate) (response sshserver.AuthResponse, metadata map[string]string, reason error) {
	return m.backend.OnAuthPubKey(username, pubKey, clientVersion, caCert)
}
