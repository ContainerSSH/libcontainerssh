...
func (t *testAuthenticationNetworkHandler) OnAuthPubKey(username string, pubKey string, _ string, _ *CACertificate) (response AuthResponse, metadata map[string]string, reason error) {
	for _, user := range t.rootHandler.users {
		if user.username == username {
