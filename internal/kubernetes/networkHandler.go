...
func (n *networkHandler) OnAuthPubKey(username string, pubKey string, clientVersion string, caKey *sshserver.CACertificate) (response sshserver.AuthResponse, metadata map[string]string, reason error) {
	return sshserver.AuthResponseUnavailable, nil, fmt.Errorf("the backend handler does not support authentication")
}
